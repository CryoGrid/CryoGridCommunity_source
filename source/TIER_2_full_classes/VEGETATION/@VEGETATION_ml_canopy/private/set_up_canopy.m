...
for p = 1:1 %vegetation.params.npts number of grid points = 1
    ...
    %Add Sebastian
    veg_layer_thickness = ground.STATVAR.vegetation.mlcanopyinst.ztop(p) ./ (ground.STATVAR.vegetation.mlcanopyinst.nveg(p)+1);
    %End ADD SEBASTIAN
    ...
    for ic = ground.STATVAR.vegetation.canopy.ntop(p)-1: -1: ground.STATVAR.vegetation.canopy.nbot(p)
        ...
        ground.STATVAR.vegetation.mlcanopyinst.zw(p,ic) = ground.STATVAR.vegetation.mlcanopyinst.zw(p,ic+1) - veg_layer_thickness;
        %END CHANGE SEBATSIAN
    end
...
%     lai_inc = dz_to_zref / n_to_zref;
    n_to_zref = round(dz_to_zref / veg_layer_thickness);
    above_veg_layer_thickness = dz_to_zref / n_to_zref;
    %END CHANGE SEBATSIAN
    ...
    for ic = ground.STATVAR.vegetation.mlcanopyinst.ncan(p)-1: -1: ground.STATVAR.vegetation.canopy.ntop(p)+1
        ...
        ground.STATVAR.vegetation.mlcanopyinst.zw(p,ic) = ground.STATVAR.vegetation.mlcanopyinst.zw(p,ic+1) - above_veg_layer_thickness;   
    end
