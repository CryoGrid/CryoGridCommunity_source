...
%             ground.PARA.end_year = datevec(tile.FORCING.PARA.end_time);
        ...
        function ground = provide_STATVAR(ground)
            ...
            ground.STATVAR.total_whc = zeros(ground.PARA.tyears,ground.PARA.nop);%intialize total water holding capacity for every patch
            ground.STATVAR.mx= zeros(1,ground.PARA.nop);%intialize total water holding capacity for every patch
            ground.STATVAR.peatD_all= 0;%intialize peatdepth of moss
            ground.STATVAR.twater= zeros(ground.PARA.tyears,ground.PARA.nop); %intialize total water in peat
            ground.STATVAR.tpeat= zeros(ground.PARA.tyears,ground.PARA.nop);%zeros(1,ground.PARA.nop); %intialize total peat
            
            ground.STATVAR.peat_moss = zeros(ground.PARA.tyears,ground.PARA.nop);
            ground.STATVAR.peat_shrub = zeros(ground.PARA.tyears,ground.PARA.nop);
            ground.STATVAR.peat_graminoid = zeros(ground.PARA.tyears,ground.PARA.nop);
            ground.STATVAR.total_peat = zeros(ground.PARA.tyears,ground.PARA.nop);
            ground.STATVAR.max_waterHoldingCapacity= zeros(ground.PARA.tyears,ground.PARA.nop);
            ground.STATVAR.peat_depth = zeros (ground.PARA.tyears,ground.PARA.nop);
...
%             ground.STATVAR.snowa = zeros(1,ground.PARA.totaldays);
            ...
            ground.STATVAR.mxx= zeros(1,ground.PARA.nop);
            ground.STATVAR.wtde = zeros(ground.PARA.tyears,ground.PARA.nop);
            ground.STATVAR.difcuu = zeros(1,ground.PARA.tyears);
            ground.STATVAR.total_cumulative_peat = zeros(ground.PARA.tyears,ground.PARA.nop);
            ...
            ground.STATVAR.catos= zeros (ground.PARA.tyears,ground.PARA.nop);
            ground.STATVAR.cats = zeros (ground.PARA.tyears,ground.PARA.nop);
            ground.STATVAR.catom= zeros (ground.PARA.tyears,ground.PARA.nop);
            ground.STATVAR.catm = zeros (ground.PARA.tyears,ground.PARA.nop);
            ground.STATVAR.catog= zeros (ground.PARA.tyears,ground.PARA.nop);
            ground.STATVAR.catg = zeros (ground.PARA.tyears,ground.PARA.nop);
            %ground.STATVAR.decdepth= zeros(ground.PARA.tyears,ground.PARA.nop);
            %ground.STATVAR.soiltd = zeros (ground.PARA.totaldays,ground.PARA.nop);
            %ground.STATVAR.mw = zeros (ground.PARA.totaldays,ground.PARA.nop);
            ground.STATVAR.peataccu = zeros (ground.PARA.tyears,ground.PARA.nop);
            ground.STATVAR.awater_depth = zeros (ground.PARA.tyears,ground.PARA.nop);
            ground.STATVAR.fpcgg = zeros (ground.PARA.tyears,ground.PARA.nop);
            ground.STATVAR.fpcss  = zeros (ground.PARA.tyears,ground.PARA.nop);
            ground.STATVAR.fpcmm  = zeros (ground.PARA.tyears,ground.PARA.nop);
            
            ground.STATVAR.fpm = zeros (ground.PARA.tyears,ground.PARA.nop);
            ground.STATVAR.fpg = zeros (ground.PARA.tyears,ground.PARA.nop);
            ground.STATVAR.fps  = zeros (ground.PARA.tyears,ground.PARA.nop);
            ground.STATVAR.T  = ones (ground.PARA.tyears,ground.PARA.nop).*5;
            %ground.STATVAR.water  = zeros (ground.PARA.tyears,ground.PARA.nop);
            
            ground.STATVAR.totalpeatC = zeros (ground.PARA.tyears,ground.PARA.nop);
            ground.STATVAR.totalpeatC_originalMass = zeros (ground.PARA.tyears,ground.PARA.nop);
            ground.STATVAR.massRemain = zeros (ground.PARA.tyears,ground.PARA.nop);
            ground.STATVAR.tempModifier = zeros (ground.PARA.tyears,ground.PARA.nop);
            
            
            ground.STATVAR.waterContent = ones(ground.PARA.tyears,ground.PARA.nop)*0.5;
            ground.STATVAR.waterModifier = zeros (ground.PARA.tyears,ground.PARA.nop);
            ground.STATVAR.cato= zeros (ground.PARA.tyears,ground.PARA.nop);
            ground.STATVAR.bulkDensity= ones (ground.PARA.tyears,ground.PARA.nop)*105;

            ground.STATVAR.layerThick = zeros (ground.PARA.tyears,ground.PARA.nop);%[0.01; 0.2; 0.2];
            ground.STATVAR.upperPosition_gridCell = zeros (ground.PARA.tyears,ground.PARA.nop);%[0; 0.01; 0.21]; %upper position of each grid cell, not used now, but will get relevant for Xice
            ground.STATVAR.area = ones (ground.PARA.tyears,ground.PARA.nop);
            ground.STATVAR.XwaterIce = zeros (ground.PARA.tyears,ground.PARA.nop);
            ground.STATVAR.organic = zeros (ground.PARA.tyears,ground.PARA.nop);
            ground.STATVAR.mineral = zeros (ground.PARA.tyears,ground.PARA.nop);
            ground.STATVAR.energy = zeros (ground.PARA.tyears,ground.PARA.nop);
            ground.STATVAR.waterIce = zeros (ground.PARA.tyears,ground.PARA.nop);
