%function  = get_boundary_condition_u_compiled(, )

%out: : surf_T, melt, snowfall, T, TEMP: d_energy, d_new_snow_layerThick

function [surf_T, melt, snowfall, T, d_energy, d_new_snow_layerThick, d_new_melt_layerThick] = get_boundary_condition_u_compiled(T, surfT, ensemble_size, melt_bare, melt_fraction, ...
melt_forest, day_sec, timestep, snow_fall, snowfall_factor, ice_snow, upper_cell, d_energy, snow_mat1, L_f, c_i, layerThick_snow, wind_speed_class)
...
surf_T = repmat(surfT, 1, ensemble_size);

melt = repmat(melt_bare, 1, ensemble_size) .* (1 - melt_fraction) + repmat(melt_forest, 1, ensemble_size) .* melt_fraction;
melt = melt ./ 1000 ./ day_sec .* timestep;  %in [m], constant timestep


snowfall = repmat(snow_fall, 1, ensemble_size) ./1000 ./ day_sec .* snowfall_factor .* timestep;
