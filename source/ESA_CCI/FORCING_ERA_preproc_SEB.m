...
classdef FORCING_ERA_preproc_SEB < matlab.mixin.Copyable
...
%             forcing.TEMP.ERA_Lin_downscaled = [];
%             forcing.TEMP.ERA_Sin_downscaled = [];
%             forcing.TEMP.ERA_precip_downcaled=[];
            
            %forcing.TEMP.ERA_time=single(zeros(size(tile.RUN_INFO.STATVAR.latitude,1),36));
            forcing.TEMP.ERA_T_downscaled=single(zeros(size(tile.RUN_INFO.STATVAR.latitude,1),36));
            forcing.TEMP.ERA_Lin_downscaled = single(zeros(size(tile.RUN_INFO.STATVAR.latitude,1),36));
            forcing.TEMP.ERA_Sin_downscaled = single(zeros(size(tile.RUN_INFO.STATVAR.latitude,1),36));
            forcing.TEMP.ERA_precip_downcaled=single(zeros(size(tile.RUN_INFO.STATVAR.latitude,1),36));
            ...
            for time = tile.t:tile.t + tile.timestep %9 days
                ...
                filename_PLEV_RH(24:27) = num2str(year); 
                ...
                if exist([forcing.PARA.ERA_path filename_SURF_airT])==2
                    ...
                    %z_1000 = ncread_with_360_degree_pl(forcing, [forcing.PARA.ERA_path filename_PLEV_airT], 't', time_index_start, 4, 1);
...
%                     vp=RH2vp(RH,T-Tm);
                    ...
                    for i=1:4
                        ...
                        interpolated_RH_surface = interp2(forcing.TEMP.ERA_lon_mesh, forcing.TEMP.ERA_lat_mesh, RH_ERA_surface_slice', forcing.TEMP.target_lon, tile.RUN_INFO.STATVAR.latitude, 'cubic');
                        lr_slice=squeeze(lapse_rate_RH(:,:,i));
                        interpolated_lapse_rate = interp2(forcing.TEMP.ERA_lon_mesh, forcing.TEMP.ERA_lat_mesh, lr_slice', forcing.TEMP.target_lon, tile.RUN_INFO.STATVAR.latitude, 'cubic');
                        
                        RH_downscaled =  interpolated_RH_surface + interpolated_lapse_rate .* (tile.RUN_INFO.STATVAR.altitude - forcing.TEMP.interpolated_ERA_orography);
...
%                         emcsi(1)
%                         emcs(1)
%                         interpolated_RH_surface(1)
%                         RH_downscaled(1)
                        
                        Lin_ERA_slice = squeeze(Lin_ERA(:,:,i));
                        interpolated_Lin = interp2(forcing.TEMP.ERA_lon_mesh, forcing.TEMP.ERA_lat_mesh, Lin_ERA_slice', forcing.TEMP.target_lon, tile.RUN_INFO.STATVAR.latitude, 'cubic');
                        ...
                        interpolated_Sin = interp2(forcing.TEMP.ERA_lon_mesh, forcing.TEMP.ERA_lat_mesh, Sin_ERA_slice', forcing.TEMP.target_lon, tile.RUN_INFO.STATVAR.latitude, 'cubic');
                        TOA_ERA_slice = squeeze(TOA_ERA(:,:,i));
                        interpolated_TOA = interp2(forcing.TEMP.ERA_lon_mesh, forcing.TEMP.ERA_lat_mesh, TOA_ERA_slice', forcing.TEMP.target_lon, tile.RUN_INFO.STATVAR.latitude, 'cubic');
                        ...
                        p1_over_p2_interp = (1 + interpolated_lapse_rate .* (tile.RUN_INFO.STATVAR.altitude - forcing.TEMP.interpolated_ERA_orography) ./ interpolated_T2m) .^(M.*g./R./-interpolated_lapse_rate);
...
%                         forcing.TEMP.ERA_Lin_downscaled = [forcing.TEMP.ERA_Lin_downscaled Lin_downscaled];
%                         forcing.TEMP.ERA_Sin_downscaled = [forcing.TEMP.ERA_Sin_downscaled Sin_downscaled];
...
%                     forcing.TEMP.ERA_precip_downcaled =[forcing.TEMP.ERA_precip_downcaled repmat(0,size(tile.RUN_INFO.STATVAR.key,1), 4)];
                    ...
                    forcing.TEMP.ERA_T_downscaled(:,count+1:count+4)= repmat(273.15-10,size(tile.RUN_INFO.STATVAR.key,1), 4);
                    forcing.TEMP.ERA_precip_downcaled(:,count+1:count+4) = repmat(0,size(tile.RUN_INFO.STATVAR.key,1), 4);
