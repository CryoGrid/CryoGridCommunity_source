classdef TTOP_Globpermafrost_ESA_CCI < BASE
    ...
    methods
        ...
        function TTOP = get_TTOP_from_forcing(ground, tile)
            ...
            shift_yes = tile.RUN_INFO.STATVAR.latitude(tile.PARA.range)>0; %Northern hemisphere
...
%             FDD = sum(surf_T .*double(surf_T<0),2);
%             TDD = sum(surf_T .*double(surf_T>0),2);
...
%             TDD = repmat(TDD', 1, tile.ENSEMBLE.PARA.ensemble_size);
            Jan_av_T = repmat(Jan_av_T', 1, tile.ENSEMBLE.PARA.ensemble_size);
            ...
            for i=1:size(snowfall,2)
                T = repmat(surf_T(:,i)', 1, tile.ENSEMBLE.PARA.ensemble_size);
                sf = repmat(snowfall(:,i)', 1, tile.ENSEMBLE.PARA.ensemble_size) .* tile.ENSEMBLE.STATVAR.snowfall_factor;
                melt = repmat(melt_bare(:,i)', 1, tile.ENSEMBLE.PARA.ensemble_size) .* (1 - tile.ENSEMBLE.STATVAR.melt_fraction) + ...
                    repmat(melt_forest(:,i)', 1, tile.ENSEMBLE.PARA.ensemble_size) .* tile.ENSEMBLE.STATVAR.melt_fraction;
            ...
            TTOP = get_GlobPF_TTOP(ground, FDD_acc, TDD_acc, swe_av ./ 1000, Jan_av_T, snow_cover_duration_av, tile.ENSEMBLE.STATVAR.rk_init, size(snowfall,2));
