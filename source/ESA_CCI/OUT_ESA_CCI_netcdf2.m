classdef OUT_ESA_CCI_netcdf2 < matlab.mixin.Copyable
        ...
        function out = finalize_init(out, tile)
            ...
            for i=1:size(variable_names,1)
                %if ~(exist([out.PARA.out_folder 'out_' num2str(tile.PARA.range(1,1)) '_' num2str(tile.PARA.range(end,1)) '.nc'])==2)
                    nccreate([out.PARA.out_folder 'out_' num2str(tile.PARA.range(1,1)) '_' num2str(tile.PARA.range(end,1)) '.nc'], variable_names{i,1}, 'datatype', 'single', 'Dimensions', ...
                        {'x',size(tile.PARA.range,1) ,'y', number_of_years, 'z', tile.ENSEMBLE.PARA.ensemble_size}, 'FillValue', -9999);
                %end
            end
