...
for i=1:size(out.STRATIGRAPHY,2)
    ...
    for j = 1:size(out.STRATIGRAPHY{1,i},1)
     ...
     for k=1:numberOfVariables
         if strcmp(variableList{k,1}, 'water') || strcmp(variableList{k,1}, 'ice') || strcmp(variableList{k,1}, 'waterIce') || strcmp(variableList{k,1}, 'saltConc')
             temp(:,k) = temp(:,k)./layerThick./area;
         end
         result.(variableList{k,1}) = [result.(variableList{k,1}) temp(:,k)];
     end
