...
classdef GROUND_freeW_bucketW_seb_snow < GROUND_freeW_bucketW_seb
    ...
    methods
        ...
        function self = GROUND_freeW_bucketW_seb_snow(index, pprovider, cprovider, forcing)
            self@GROUND_freeW_bucketW_seb(index, pprovider, cprovider, forcing);
        end
       ...
       function ground = finalize_init(ground, forcing)
           ground = finalize_init@GROUND_freeW_bucketW_seb(ground, forcing);
       ...
       %-------------------
        ...
        function ground = get_boundary_condition_u(ground, forcing)
            ...
            if ground.CHILD == 0  %CHILD does not exist
                ground = get_boundary_condition_u@GROUND_freeW_bucketW_seb(ground, forcing); %call the native function for the ground class
...
%                 get_IA_CHILD_boundary_condition_u(ground.IA_CHILD); %should require the full area of the ground class
        ...
        function ground = get_boundary_condition_l(ground, forcing)
              ground = get_boundary_condition_l@GROUND_freeW_bucketW_seb(ground, forcing);
        ...
        function ground = get_derivatives_prognostic(ground)
            if ground.CHILD == 0  
                ground = get_derivatives_prognostic@GROUND_freeW_bucketW_seb(ground); %call normal function
            else
                ground.CHILD = get_derivatives_prognostic_CHILD(ground.CHILD);
                ground = get_derivatives_prognostic@GROUND_freeW_bucketW_seb(ground); 
            end
        end
        
        function timestep = get_timestep(ground) 
            if ground.CHILD == 0
                timestep = get_timestep@GROUND_freeW_bucketW_seb(ground);
            else 
                timestep_snow = get_timestep_CHILD(ground.CHILD);
                timestep_ground =  get_timestep@GROUND_freeW_bucketW_seb(ground);
        ...
        function ground = advance_prognostic(ground, timestep) %real timestep derived as minimum of several classes in [sec] here!
            if ground.CHILD == 0
                ground =  advance_prognostic@GROUND_freeW_bucketW_seb(ground, timestep);
            else                
        ...
        function ground = compute_diagnostic(ground, forcing)
            if ground.CHILD == 0
                ...
                ground = compute_diagnostic@GROUND_freeW_bucketW_seb(ground, forcing);
            else
