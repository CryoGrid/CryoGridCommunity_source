...
classdef GROUND_freezeC_seb_snow < GROUND_freezeC_seb
    ...
    methods
       ...
       function ground = finalize_init(ground, forcing)
           ground = finalize_init@GROUND_freezeC_seb(ground, forcing);
        ...
        function ground = get_boundary_condition_u(ground, forcing)
            ...
            if ground.CHILD == 0  %CHILD does not exist
                ground = get_boundary_condition_u@GROUND_freezeC_seb(ground, forcing); %call the native function for the ground class
                ...
                ground = get_boundary_condition_u@GROUND_freezeC_seb(ground, forcing);
        ...
        function ground = get_boundary_condition_l(ground, forcing)
              ground = get_boundary_condition_l@GROUND_freezeC_seb(ground, forcing);
        ...
        function ground = get_derivatives_prognostic(ground)
            if ground.CHILD == 0  
                ground = get_derivatives_prognostic@GROUND_freezeC_seb(ground); %call normal function
            else
                ground.CHILD = get_derivatives_prognostic_CHILD(ground.CHILD);
                ground = get_derivatives_prognostic@GROUND_freezeC_seb(ground); 
            end
        end
        
        function timestep = get_timestep(ground) 
            if ground.CHILD == 0
                timestep = get_timestep@GROUND_freezeC_seb(ground);
            else 
                timestep_snow = get_timestep_CHILD(ground.CHILD);
                timestep_ground =  get_timestep@GROUND_freezeC_seb(ground);
        ...
        function ground = compute_diagnostic(ground, forcing)
            if ground.CHILD == 0
                ground = compute_diagnostic@GROUND_freezeC_seb(ground, forcing);
            else
                ground = compute_diagnostic@GROUND_freezeC_seb(ground, forcing);
