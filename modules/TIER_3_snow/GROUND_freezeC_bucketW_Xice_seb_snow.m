...
classdef GROUND_freezeC_bucketW_Xice_seb_snow < GROUND_freezeC_bucketW_Xice_seb
    ...
    methods
        ...
        function ground = GROUND_freezeC_bucketW_Xice_seb_snow(index, pprovider, cprovider, forcing)
            ground@GROUND_freezeC_bucketW_Xice_seb(index, pprovider, cprovider, forcing);
        end
        ...
        function ground = finalize_init(ground, forcing)
            ground = finalize_init@GROUND_freezeC_bucketW_Xice_seb(ground, forcing);
        ...
        function ground = get_boundary_condition_u(ground, forcing)
            ...
            if ground.CHILD == 0  %CHILD does not exist
                ground = get_boundary_condition_u@GROUND_freezeC_bucketW_Xice_seb(ground, forcing); %call the native function for the ground class
                ...
                ground = get_boundary_condition_u@GROUND_freezeC_bucketW_Xice_seb(ground, forcing);
        ...
        function ground = get_boundary_condition_l(ground, forcing)
            ground = get_boundary_condition_l@GROUND_freezeC_bucketW_Xice_seb(ground, forcing);
        end
        
        
        function ground = get_derivatives_prognostic(ground)
            if ground.CHILD == 0
                ground = get_derivatives_prognostic@GROUND_freezeC_bucketW_Xice_seb(ground); %call normal function
            else
                ground.CHILD = get_derivatives_prognostic_CHILD(ground.CHILD);
                ground = get_derivatives_prognostic@GROUND_freezeC_bucketW_Xice_seb(ground);
            end
        end
        
        function timestep = get_timestep(ground)
            if ground.CHILD == 0
                timestep = get_timestep@GROUND_freezeC_bucketW_Xice_seb(ground);
            else
                timestep_snow = get_timestep_CHILD(ground.CHILD);
                timestep_ground =  get_timestep@GROUND_freezeC_bucketW_Xice_seb(ground);
        ...
        function ground = advance_prognostic(ground, timestep)
            if ground.CHILD == 0
                ground =  advance_prognostic@GROUND_freezeC_bucketW_Xice_seb(ground, timestep);
            else
        ...
        function ground = compute_diagnostic(ground, forcing)
            if ground.CHILD == 0
                ground = compute_diagnostic@GROUND_freezeC_bucketW_Xice_seb(ground, forcing);
            else
        ...
        function ground = check_trigger(ground, forcing)
            ...
            ground = check_trigger@GROUND_freezeC_bucketW_Xice_seb(ground, forcing);
