...
%             D = tile.SUBSURFACE_CLASS.STATVAR.K_grid(2:end,:) - profile.K_grid(1:end-1,:); %N
            thaw_depth = sum(tile.SUBSURFACE_CLASS.STATVAR.layerThick(5:end,:) .* AL_cell, 1) .* double(tile.SUBSURFACE_CLASS.STATVAR.T(5,:)>0);
            out.TEMP.acc_max_ALT = max(out.TEMP.acc_max_ALT, thaw_depth);
            
            out.TEMP.acc_snow_density = out.TEMP.acc_snow_density + sum(tile.SUBSURFACE_CLASS.STATVAR.ice_snow,1)./sum(max(1e-20, tile.SUBSURFACE_CLASS.STATVAR.layerThick_snow),1).*917; 
            ...
            out.TEMP.acc_snow_covered_days = out.TEMP.acc_snow_covered_days + double(sum(tile.SUBSURFACE_CLASS.STATVAR.layerThick_snow,1)>0);
             ...
             if tile.t >= out.OUTPUT_TIME 
                 ...
                 out.STATVAR.T5m = [out.STATVAR.T5m; out.TEMP.acc_T2m ./ out.TEMP.acc_N];
                 ...
                 out.STATVAR.T1m_std = [out.STATVAR.T1m_std; sqrt(out.TEMP.acc_T1m_square./out.TEMP.acc_N - (out.TEMP.acc_T1m ./ out.TEMP.acc_N).^2)];
                 out.STATVAR.T2m_std = [out.STATVAR.T2m_std; sqrt(out.TEMP.acc_T2m_square./out.TEMP.acc_N - (out.TEMP.acc_T2m ./ out.TEMP.acc_N).^2)];
                 ...
                 if tile.t>=out.SAVE_TIME
                     ...
                     filename =  [out.PARA.out_folder tile.RUN_INFO.PARA.run_name '/'  'out_' datestr(tile.t, 'yyyy') '_' num2str(tile.PARA.range(1,1)) '_' num2str(tile.PARA.range(end,1)) '.mat'];
                     var = out.STATVAR;
                     save(filename, 'var');
                     
%                      for i=1:size(variable_names,1)
%                      
% %                          filename =  [out.PARA.out_folder tile.RUN_INFO.PARA.run_name '/' variable_names{i,1} '_' num2str(tile.PARA.range(1,1)) '_' num2str(tile.PARA.range(end,1)) '.mat'];
% %                          var = out.STATVAR.(variable_names{i,1});
%                          filename =  [out.PARA.out_folder tile.RUN_INFO.PARA.run_name '/' variable_names{i,1} '_' num2str(tile.PARA.range(1,1)) '_' num2str(tile.PARA.range(end,1)) '.mat'];
%                          var = out.STATVAR;
%                          save(filename, 'var');
%                          %make this dlmwrite
...
%         end
        ...
        function out = set2zero(out)
            ...
            out.TEMP.acc_T20m=0;
            out.TEMP.acc_GST=0;
            out.TEMP.acc_max_ALT = 0;
            out.TEMP.acc_FT_state = 0;
            out.TEMP.acc_FT_isothermal = 1;
            out.TEMP.acc_N=0;
            out.TEMP.acc_snow_density = 0;
            out.TEMP.acc_snow_depth = 0;
            out.TEMP.acc_snow_covered_days = 0;
            out.TEMP.acc_T1m_max = -1000;
            out.TEMP.acc_T1m_min = 1000;
            out.TEMP.acc_T2m_max = -1000;
            out.TEMP.acc_T2m_min = 1000;
            out.TEMP.acc_T1m_square = 0;
            out.TEMP.acc_T2m_square = 0;
            out.TEMP.acc_FDD = 0;  %initialization
            out.TEMP.acc_TDD = 0;
        end
