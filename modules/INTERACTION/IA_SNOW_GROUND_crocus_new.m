classdef IA_SNOW_GROUND_crocus_new
...
%                 if snow.TEMP.snowfall > 0
                    if isempty(snow.STATVAR.d)
                        snow.STATVAR.d = snow.TEMP.d;
                        snow.STATVAR.s = snow.TEMP.s;
                        snow.STATVAR.gs = snow.TEMP.gs;
                        snow.STATVAR.time_snowfall = snow.TEMP.time_snowfall;
                        snow.TEMP.rho_new_snow = snow.TEMP.rho_falling_snow;
                    else
                        snow.STATVAR.d = (snow.STATVAR.d*snow.STATVAR.ice + snow.TEMP.d*timestep.*snow.TEMP.snowfall)./(snow.STATVAR.ice + timestep .*snow.TEMP.snowfall);
                        snow.STATVAR.s = (snow.STATVAR.s*snow.STATVAR.ice + snow.TEMP.s*timestep.*snow.TEMP.snowfall)./(snow.STATVAR.ice + timestep .*snow.TEMP.snowfall);
                        snow.STATVAR.gs = (snow.STATVAR.gs*snow.STATVAR.ice + snow.TEMP.gs*timestep.*snow.TEMP.snowfall)./(snow.STATVAR.ice + timestep .*snow.TEMP.snowfall);
                        snow.STATVAR.time_snowfall = (snow.STATVAR.time_snowfall*snow.STATVAR.ice + snow.TEMP.time_snowfall*timestep.*snow.TEMP.snowfall)./(snow.STATVAR.ice + timestep .*snow.TEMP.snowfall);
                        snow.TEMP.rho_new_snow = (snow.TEMP.rho_new_snow*snow.STATVAR.ice + snow.TEMP.rho_falling_snow*timestep.*snow.TEMP.snowfall)./(snow.STATVAR.ice + timestep .*snow.TEMP.snowfall);
                    end
%                 end
                
                snow.STATVAR.energy = snow.STATVAR.energy + timestep .*(snow.TEMP.snow_energy + snow.TEMP.rain_energy); %only the energy from new snow
                snow.STATVAR.waterIce = snow.STATVAR.waterIce + timestep .* (snow.TEMP.snowfall + snow.TEMP.rainfall);
                snow.STATVAR.ice_fraction = (snow.STATVAR.ice + timestep .* snow.TEMP.snowfall)./ (snow.STATVAR.layerThick + timestep .* snow.TEMP.snowfall ./ (snow.TEMP.rho_new_snow./snow.CONST.rho_w));
