function pot_water_fluxes = lateral_Darcy_flux(lateral,pot_water_fluxes,j)
    ...
    if lateral.PARA.contact_length(j,index) > 0 
        ...
        if lateral.TEMP.water_table(index) > lateral.TEMP.water_table(j) && sum(lateral.TEMP.mobile_water(index)) > 0 % current worker is sending water
            ...
            pot_water_fluxes(index,j) = Darcy_flux * lateral.PARA.interaction_timestep * 3600 / lateral.PARA.area(index); % m potential water change
            pot_water_fluxes(j,index) = -1* Darcy_flux * lateral.PARA.interaction_timestep * 3600 / lateral.PARA.area(j);
        elseif lateral.TEMP.water_table(index) < lateral.TEMP.water_table(j) && sum(lateral.TEMP.mobile_water(j)) > 0 % receiving water
            ...
            pot_water_fluxes(index,j) = -1* Darcy_flux * lateral.PARA.interaction_timestep * 3600 / lateral.PARA.area(index);
            pot_water_fluxes(j,index) = Darcy_flux * lateral.PARA.interaction_timestep * 3600 / lateral.PARA.area(j);
        else % No water flux
