...
classdef GROUND_fcSimple_salt_seb_snow_test < GROUND_fcSimple_salt_seb
    properties
        IA_CHILD
    end
       ...
       function ground = provide_variables(ground)  %initializes the subvariables as empty arrays
        ...
        function ground = get_boundary_condition_u(ground, forcing)
            ...
            if ground.IA_CHILD.STATUS == 0 && forcing.TEMP.snowfall > 0  %zero SWE and snowfall occuring, snow CHILD must be initialized
               ...
               ground = get_boundary_condition_u@GROUND_fcSimple_salt_seb(ground, forcing); %call the native function for the ground class
...
%                 end
        ...
        function ground = get_boundary_condition_l(ground, forcing)
              ground = get_boundary_condition_l@GROUND_fcSimple_salt_seb(ground, forcing);
        ...
        function ground = get_derivatives_prognostic(ground)
            if ground.IA_CHILD.STATUS <= 1  
                ground = get_derivatives_prognostic@GROUND_fcSimple_salt_seb(ground); %call normal function
            else
                if ground.IA_CHILD.STATUS == 2 %non-zero SWE, but snow is still a child
                    ...
                    ground = get_derivatives_prognostic@GROUND_fcSimple_salt_seb(ground); %call normal function for ground
                else
                    ground = get_derivatives_prognostic@GROUND_fcSimple_salt_seb(ground); %call normal function
                end
            end
        ...
        function timestep = get_timestep(ground) 
            if ground.IA_CHILD.STATUS <= 1  
                ...
                timestep_ground =  get_timestep@GROUND_fcSimple_salt_seb(ground);
        ...
        function ground = compute_diagnostic(ground, forcing)
            if ground.IA_CHILD.STATUS <= 0
                ground = compute_diagnostic@GROUND_fcSimple_salt_seb(ground, forcing);
            else
