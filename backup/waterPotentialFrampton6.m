...
%%
sat_waterIce=[0.01:0.01:1]'
mwp0 = 1./alpha .* ((sat_waterIce.^(-1./m)-1)).^(1./n);

plot(sat_waterIce, mwp0./1000/9.81)
%%
close all
        ...
        for sat_waterIce = 0.005:0.001:0.15
        %sat_waterIce = 0.01;
        for theta_o = 0.05:0.025:0.95
        %theta_o = 0.05;
        theta_m = 0.;
...
%         plot(test_T, T_test)
       ...
       % plot(sat_water_test, (sat_water_test - sat_water_final) ./ sat_water_test)
%         plot(test_T, T_final)
     ...
     %   figure
       % plot(sat_water_test, (sat_water_test - sat_water_final_alternative) ./ sat_water_test)
...
%         
        store = [store; [gamma dec nanmean((sat_water_test - sat_water_final) ./ sat_water_test) nanmean((sat_water_test - sat_water_final_alternative) ./ sat_water_test) mean(test_T - T_final) mean(test_T - T_final_alternative)]];
            
            end
            end
        %         log(dec)
%         -0.34.* log(gamma)+7.2
%         %non-normal solution preferreed iun this case
%         real(log(dec)) > real(-0.362.* log(-gamma)+6.65)


% test = store(find(abs(store(:,4)) <= abs(store(:,3)) & abs(store(:,6)) <= abs(store(:,5))),:);
% test2 = test(find(abs(test(:,4)).*5 >= abs(test(:,3)) & abs(test(:,6)).*5 >= abs(test(:,5))),:);
% plot(log(-test2(:,1)), log(test2(:,2)), '+')
%fit this realtionship to find a  distinguishing linear relationship
...
% store=[];
% 
% for theta = [0.01:0.01:0.5]
% 
% dpot_dtheta = 1./alpha ./ P ./ (-m) .* (theta./P).^(-1/m-1) ./n .*((theta./P).^(-1/m)-1).^(1./n-1);
% 
% hk = hydraulicConductivity .* (theta./P).^0.5 .* (1 - (1 - (theta./P).^(n./(n+1))).^(1-1./n)).^2;
% 
% store = [store; [dpot_dtheta hk dpot_dtheta.*hk 0.01.^2 ./ (2.*dpot_dtheta.*hk)]];
% end